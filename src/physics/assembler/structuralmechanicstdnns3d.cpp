...
#include "../assembler/structuralmechanicstdnns3d.h"
...
StructuralMechanicsTDNNS3D::StructuralMechanicsTDNNS3D(Mesh *mesh, Instance *instance, Step *step, const StructuralMechanicsConfiguration &configuration, const ResultsSelectionConfiguration &propertiesConfiguration)
: Physics("STRUCTURAL MECHANICS TDNNS 3D", mesh, instance, step, &configuration, 3), StructuralMechanics(configuration, propertiesConfiguration, 3)
{

}

void StructuralMechanicsTDNNS3D::analyticRegularization(size_t domain, bool ortogonalCluster)
{
...
void StructuralMechanicsTDNNS3D::processBEM(eslocal domain, Matrices matrices)
{

}

void StructuralMechanicsTDNNS3D::assembleMaterialMatrix(eslocal node, const Point &p, const MaterialBaseConfiguration *mat, double temp, DenseMatrix &K) const
{
...
void StructuralMechanicsTDNNS3D::processElement(eslocal domain, Matrices matrices, eslocal eindex, DenseMatrix &Ke, DenseMatrix &Me, DenseMatrix &Re, DenseMatrix &fe) const
{
	...
	// BASE function derivations
	const std::vector<DenseMatrix> &dN = *(epointer->dN);
	const std::vector<double> &weighFactor = *(epointer->weighFactor);

	std::cout << "ELEMENTS FACES:\n";
	auto facepointer = epointer->facepointers->begin();
	for (auto face = epointer->faces->begin(); face != epointer->faces->end(); ++face, ++facepointer) {
		std::cout << "CODE (" << (int)facepointer->front()->code << ") : ";
		for (auto fnode = face->begin(); fnode != face->end(); ++fnode) {
			std::cout << nodes->at(*fnode) << " ";
		}
		std::cout << "\n";
	}
...
void StructuralMechanicsTDNNS3D::processFace(eslocal domain, const BoundaryRegionStore *region, Matrices matrices, eslocal findex, DenseMatrix &Ke, DenseMatrix &Me, DenseMatrix &Re, DenseMatrix &fe) const
{
...
void StructuralMechanicsTDNNS3D::processEdge(eslocal domain, const BoundaryRegionStore *region, Matrices matrices, eslocal eindex, DenseMatrix &Ke, DenseMatrix &Me, DenseMatrix &Re, DenseMatrix &fe) const
{
	Ke.resize(0, 0);
	Me.resize(0, 0);
	Re.resize(0, 0);
...
void StructuralMechanicsTDNNS3D::processNode(eslocal domain, const BoundaryRegionStore *region, Matrices matrices, eslocal nindex, DenseMatrix &Ke, DenseMatrix &Me, DenseMatrix &Re, DenseMatrix &fe) const
{
//	if (
...
void StructuralMechanicsTDNNS3D::postProcessElement(eslocal eindex)
{

}

void StructuralMechanicsTDNNS3D::processSolution()
{
}
