...
CollectiveProvider::CollectiveProvider(Instance &instance, Physics &physics, Composer &composer, Mesh &mesh, Step &step, ResultStore &store, LinearSolver &linearSolver)
...
CollectiveProvider::~CollectiveProvider()
{

}

void CollectiveProvider::preprocessData()
{
...
void CollectiveProvider::updateStructuralMatrices(Matrices matrices)
{
...
void CollectiveProvider::solve(Matrices updatedMatrices)
{
...
void CollectiveProvider::sum(std::vector<std::vector<double> > &z, double a, const std::vector<std::vector<double> > &x, double b, const std::vector<std::vector<double> > &y, const std::string &description)
{
...
void CollectiveProvider::sum(std::vector<std::vector<double> > &z, double a, const std::vector<std::vector<double> > &x, double b, const std::vector<std::vector<double> > &y, const std::vector<size_t> &prefix, const std::string &description)
{
...
void CollectiveProvider::sum(std::vector<SparseMatrix> &A, double beta, std::vector<SparseMatrix> &B, const std::string &description)
{
	timeWrapper("compute: " + description, [&] () {
		#pragma omp parallel for
		for (size_t d = 0; d < instance.domains; d++) {
...
/// y = A * x
void CollectiveProvider::multiply(std::vector<std::vector<double> > &y, std::vector<SparseMatrix> &A, std::vector<std::vector<double> > &x, const std::string &description)
{
	timeWrapper("compute: " + description, [&] () {
		#pragma omp parallel for
		for (size_t d = 0; d < instance.domains; d++) {
...
// v = x * y
double CollectiveProvider::multiply(std::vector<std::vector<double> > &x, std::vector<std::vector<double> > &y, const std::string &description)
{
	double sum = 0;
	timeWrapper("compute: " + description, [&] () {
		double psum = 0;
...
double CollectiveProvider::sumSquares(const std::vector<std::vector<double> > &data, SumRestriction restriction, const std::string &description)
{
	double result;
...
void CollectiveProvider::addToDirichletInB1(double a, const std::vector<std::vector<double> > &x)
{
...
double CollectiveProvider::maxAbsValue(const std::vector<std::vector<double> > &v, const std::string &description)
{
	double gmax;
	timeWrapper(description, [&] () {
		double max = 0;
...
double CollectiveProvider::lineSearch(const std::vector<std::vector<double> > &U, std::vector<std::vector<double> > &deltaU, std::vector<std::vector<double> > &F_ext)
{
	double alpha = 1;
...
void CollectiveProvider::setRegularizationCallback()
{
...
void CollectiveProvider::setRegularizationFromOrigKCallback()
{
...
void CollectiveProvider::setEmptyRegularizationCallback()
{
	instance.N1.clear();
	instance.N2.clear();
...
void CollectiveProvider::setB0Callback()
{
