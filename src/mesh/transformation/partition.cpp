...
//
//void Transformation::reclusterize(Mesh &mesh)
//{
...
//}
//
//void Transformation::partitiate(Mesh &mesh, esglobal parts, TFlags::SEPARATE separate)
//{
...
//}
//
//void Transformation::permuteElements(Mesh &mesh, const std::vector<eslocal> &permutation, const std::vector<size_t> &distribution)
//{
...
//	std::vector<std::vector<std::pair<esglobal, esglobal> > > rHalo(mesh.neighbours.size());
//
//	if (mesh._elems->dual != NULL || mesh._nodes->elems != NULL) {
//		// thread x neighbor x elements(oldID, newID)
//		std::vector<std::vector<std::vector<std::pair<esglobal, esglobal> > > > sHalo(threads, std::vector<std::vector<std::pair<esglobal, esglobal> > >(mesh.neighbours.size()));
//
//		#pragma omp parallel for
//		for (size_t t = 0; t < threads; t++) {
...
//
//		if (!Communication::exchangeUnknownSize(sHalo[0], rHalo, mesh.neighbours)) {
...
//}
//
//void Transformation::exchangeElements(Mesh &mesh, const std::vector<esglobal> &partition)
//{
...
//	std::vector<std::vector<std::vector<std::pair<esglobal, esglobal> > > > sHaloTarget(threads, std::vector<std::vector<std::pair<esglobal, esglobal> > >(mesh.neighbours.size()));
//	std::vector<std::vector<std::pair<esglobal, esglobal> > > haloTarget(mesh.neighbours.size());
//
...
//
//	if (!Communication::exchangeUnknownSize(sHaloTarget[0], haloTarget, mesh.neighbours)) {
