...
void Element::set<Element::CODE::SQUARE4>()
{
	...
	nN = new std::vector<DenseMatrix>(nodeCount, DenseMatrix(1, nodeCount));
	ndN = new std::vector<DenseMatrix>(nodeCount, DenseMatrix(2, nodeCount));

	{
		...
		for (unsigned int i = 0; i < GPCount; i++) {
			...
			(*ndN)[i](0, 2) = 0.25 * ( t[i] + 1);
			(*ndN)[i](0, 3) = 0.25 * (-t[i] - 1);

			// node dNt - derivation of basis function
			(*ndN)[i](1, 0) = 0.25 * ( s[i] - 1);
			(*ndN)[i](1, 1) = 0.25 * (-s[i] - 1);
			(*ndN)[i](1, 2) = 0.25 * ( s[i] + 1);
			(*ndN)[i](1, 3) = 0.25 * (-s[i] + 1);
		}
	}
}
}
