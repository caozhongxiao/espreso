...
void TableInterpolationEvaluator::evaluate(eslocal size, const Point* cbegin, const double* tbegin, double time, double *results) const
{
	for (eslocal i = 0; i < size; ++i) {
		if (tbegin[i] < _table[0].first) {
			results[i] = _table[0].second;
			break;
		}
		for (size_t j = 0; j < _table.size() - 1; j++) {
			if (_table[j].first < tbegin[i] && tbegin[i] < _table[j + 1].first) {
				...
				results[i] = va + (vb - va) * (tbegin[i] - a) / (b - a);
				break;
			}
		}
		results[i] = _table.back().second;
	}
}
