...
bool EBlock::solid(std::vector<eslocal> &edist, std::vector<eslocal> &enodes, std::vector<EData> &edata)
{
	...
	for (size_t t = 0; t < threads; t++) {
		edist.insert(edist.end(), tedist[t].begin(), tedist[t].end());
...
bool EBlock::boundary(std::vector<eslocal> &edist, std::vector<eslocal> &enodes, std::vector<eslocal> &edata)
{
	size_t threads = environment->OMP_NUM_THREADS;
	...
	std::vector<std::vector<eslocal> > tedist(threads), tnodes(threads), tdata(threads);
