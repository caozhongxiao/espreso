...
void GridTowerGenerator::init(const GridTowerGeneratorConfiguration &configuration)
{
	...
	for (auto grid = configuration.grids.begin(); grid != configuration.grids.end(); ++grid) {
		...
		MPI_Allreduce(&maxcluster, &cluster, 1, MPI_INT, MPI_MAX, info::mpi::comm);
		noffset += nodes;
		MPI_Allreduce(&noffset, &nodes, sizeof(size_t), MPI_BYTE, MPITools::sizetOperations().max, info::mpi::comm);
		noffset = nodes;
		...
		case GridTowerGeneratorConfiguration::DIRECTION::X:
			MPI_Allreduce(&_settings.end.x, &end, 1, MPI_INT, MPI_MAX, info::mpi::comm);
			if (grid->first < _gridIndex) {
		...
		case GridTowerGeneratorConfiguration::DIRECTION::Y:
			MPI_Allreduce(&_settings.end.y, &end, 1, MPI_INT, MPI_MAX, info::mpi::comm);
			if (grid->first < _gridIndex) {
		...
		case GridTowerGeneratorConfiguration::DIRECTION::Z:
			MPI_Allreduce(&_settings.end.z, &end, 1, MPI_INT, MPI_MAX, info::mpi::comm);
			if (grid->first < _gridIndex) {
