...
void SphereGenerator::generate(const SphereGeneratorConfiguration &configuration, Mesh &mesh)
{
	PlainMeshData meshData;
	SphereGenerator sphere(configuration);

	sphere.nodes(meshData);
	sphere.elements(meshData);
	sphere.neighbors(meshData);
	sphere.regions(configuration, meshData);
...
void SphereGenerator::nodes(PlainMeshData &mesh)
{
	GridGenerator::nodes(mesh);
	for (auto n = mesh.nIDs.begin(); n != mesh.nIDs.end(); ++n) {
		*n += environment->MPIrank * mesh.nIDs.size();
	}
}
...
void SphereGenerator::regions(const SphereGeneratorConfiguration &configuration, PlainMeshData &mesh)
{
