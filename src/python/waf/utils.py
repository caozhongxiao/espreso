...
def set_compiler_defines(ctx):
    ...
    if ctx.env.INT_WIDTH == 32:
        ctx.env.append_unique("DEFINES", [ "esint=int", "MKL_INT=int", "esint=int", "esint_mpi=MPI_INT", "BLAS_INT", "INT_WIDTH=32" ])
    elif ctx.env.INT_WIDTH == 64:
        ctx.env.append_unique("DEFINES", [ "esint=long", "MKL_INT=long", "esint=long", "esint_mpi=MPI_LONG", "BLAS_LONG", "INT_WIDTH=64" ])
    else:
